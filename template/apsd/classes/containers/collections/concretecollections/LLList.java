...
public class LLList<Data> extends LLChainBase<Data> implements List<Data> {
    ...
    @Override
    public LLChainBase<Data> NewChain(long capacity, LLNode<Data> head, LLNode<Data> tail) { return new LLList<>(capacity, head, tail); }
    ...
    public MutableForwardIterator<Data> FIterator() {
        return new MutableForwardIterator<Data>() {
            ...
            public void SetCurrent(Data dat) {
            	if(current != null) current.Set(dat);
			}
    ...
    public void InsertAt(Data dat, Natural n) {
        ...
        if(idx == 0) InsertFirst(dat); else if(idx == size.ToLong()) InsertLast(dat);
        	...
        	prev.SetNext(newNode); size.Increment();
    	}
    }
    ...
    @Override
    public boolean InsertSome(TraversableContainer<Data> c) { return InsertAll(c); }
    
    @Override
    public boolean RemoveAll(TraversableContainer<Data> c) { return false; } 
    
    @Override
    public boolean RemoveSome(TraversableContainer<Data> c) { return false; }
}