...
abstract public class LLChainBase<Data> implements Chain<Data> {
  ...
  // FIX: PUBLIC per visibilità in LLSortedChain
  ...
  public void InsertFirst(Data dat) {
      LLNode<Data> newNode = new LLNode<>(dat);
      if (IsEmpty()) { headref.Set(newNode); tailref.Set(newNode); } 
      else { newNode.SetNext(headref.Get()); headref.Set(newNode); }
      size.Increment();
  }
  ...
  @Override public Data FirstNRemove() { if (IsEmpty()) return null; Data d = GetFirst(); RemoveFirst(); return d; }
  @Override public Data LastNRemove() { if (IsEmpty()) return null; Data d = GetLast(); RemoveLast(); return d; }
  ...
  @Override public boolean IsEqual(IterableContainer<Data> c) { if(c==null || Size().compareTo(c.Size())!=0) return false; ForwardIterator<Data> it = FIterator(); return !c.TraverseForward(d -> !d.equals(it.DataNNext())); }
  @Override public boolean TraverseForward(Predicate<Data> p) { LLNode<Data> c = headref.Get(); while (c != null) { if (p.Apply(c.Get())) return true; c = c.GetNext().Get(); } return false; }
  @Override public boolean TraverseBackward(Predicate<Data> p) { return new Vector<>(this).TraverseBackward(p); }
  @Override public boolean IsInBound(Natural n) { return n.ToLong() < size.ToLong(); }
  @Override public Sequence<Data> SubSequence(Natural s, Natural e) { return null; }
  ...
  @Override public Data GetAt(Natural n) { LLNode<Data> node = getNodeAt(n.ToLong()); if (node == null) throw new IndexOutOfBoundsException(); return node.Get(); }
  @Override public boolean Filter(Predicate<Data> p) { return false; }
  @Override public boolean Exists(Data dat) { LLNode<Data> c = headref.Get(); while(c!=null) { Data d=c.Get(); if((d==null&&dat==null)||(d!=null&&d.equals(dat))) return true; c=c.GetNext().Get(); } return false; }
  ...
  @Override public boolean InsertAll(TraversableContainer<Data> c) { if(c!=null) c.TraverseForward(d->{Insert(d); return false;}); return true; }
  ...
  @Override public Natural Search(Data dat) { long i=0; LLNode<Data> c = headref.Get(); while(c!=null) { Data d=c.Get(); if((d==null&&dat==null)||(d!=null&&d.equals(dat))) return new Natural((int)i); c=c.GetNext().Get(); i++; } return null; }
}