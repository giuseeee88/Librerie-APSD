...
public class DynVector<Data> extends DynLinearVectorBase<Data> {
    ...
    public static <Data> DynVector<Data> Wrap(Data[] arr) { return new DynVector<>(arr); }
    ...
    public void ShiftRight(Natural start, Natural n) {
        ...
        Expand(n);
        long limit = size - d;
        for (long i = limit - 1; i >= s; i--) SetAt(GetAt(Natural.Of(i)), Natural.Of(i + d));
        for (long i = s; i < s + d; i++) SetAt(null, Natural.Of(i)); // Cleanup
    }
    ...
    public void ShiftLeft(Natural start, Natural n) {
        ...
        long limit = size;
        if (s + d > limit) return;
        for (long i = s; i < limit - d; i++) SetAt(GetAt(Natural.Of(i + d)), Natural.Of(i));
        for (long i = limit - d; i < limit; i++) SetAt(null, Natural.Of(i)); // Cleanup
        Reduce(n);
    }
