...
public class DynCircularVector<Data> extends DynCircularVectorBase<Data> implements DynVector<Data> {
    ...
    @Override
    public void Grow() {
        Realloc(new Natural((int) ((Capacity().ToLong() == 0 ? 1 : Capacity().ToLong() * GROW_FACTOR))));
    }

    @Override
    public void Grow(Natural n) {
        Realloc(new Natural((int) (Capacity().ToLong() + n.ToLong())));
    }

    @Override
    public void Shrink() {
        if (size < Capacity().ToLong() / SHRINK_FACTOR) Realloc(new Natural((int) size));
    }
    ...
    public void SetLast(Data dat) {
        if (size == 0) throw new IndexOutOfBoundsException();
        SetAt(dat, new Natural((int) (size - 1)));
    }

    @Override
    public Data GetNSetLast(Data dat) {
        if (size == 0) throw new IndexOutOfBoundsException();
        return GetNSetAt(dat, new Natural((int) (size - 1)));
    }
    ...
    public void InsertLast(Data dat) {
        InsertAt(dat, new Natural((int) size));
    }
    ...
    public void RemoveLast() {
        AtNRemove(new Natural((int) (size - 1)));
    }
    ...
    public Data LastNRemove() {
        return AtNRemove(new Natural((int) (size - 1)));
    }
    ...
    public void ShiftLeft(Natural n) {
        ShiftLeft(new Natural(0), n);
    }

    @Override
    public void ShiftFirstLeft() {
        ShiftLeft(new Natural(1));
    }

    @Override
    public void ShiftLastLeft() {
        ShiftLeft(new Natural((int) (size - 1)), new Natural(1));
    }

    @Override
    public void ShiftRight(Natural n) {
        ShiftRight(new Natural(0), n);
    }

    @Override
    public void ShiftFirstRight() {
        ShiftRight(new Natural(1));
    }

    @Override
    public void ShiftLastRight() {
        ShiftRight(new Natural((int) (size - 1)), new Natural(1));
    }
    ...
    public apsd.interfaces.containers.sequences.DynVector<Data> SubVector(Natural start, Natural end) {
        ...
        for (long i = 0; i < len; i++) {
            sub.rawSet(rawGet(start.ToLong() + i), i);
        }
        return sub;
    }
}